...
用户表述的天数按 SKILL.md 映射到 `--days`，正式与诊断采集均适用；本次覆盖不改写默认值或历史快照。正式关键词/地域仍需按用户的新要求调整配置和验证，不能用诊断关键词冒充正式搜索。有限诊断：`collect --days 3 --diagnostic --keyword 软件 --site ccgp --max-pages 1 --max-details 1`，执行前确保这是用户授权的诊断。
