...
| `revision.ts` | 分别比较正文抓取时间与附件归档观察，拒绝已知回退或无法排序的材料 |
...
历史快照保留当时的字段、输入哈希和分析结果；读取时校验保存字段，不用新提取器重写旧证据。重新 `--prepare` 时应用当前字段规则，规则指纹包含日期提取器与材料版本比较实现。附件观察元数据沿 P2→P3→P5 传递；附件仍待来源复核时不能视作完整材料。

`pnpm test` 覆盖解析边界、引用拒绝、版本变化、合成资质、CLI 失败、恢复、评估及附件更新通知链路。不包含政府网站的新请求，也不代表真实人工标注指标达标。
